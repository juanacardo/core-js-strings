...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < vovels.length; j += 1) {
...
function isPalindrome(str) {
  const re = /\W/g;
  const optimizedString = str.toLowerCase().replace(re, '');
  const reversedString = reverseString(optimizedString);
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < words.length - 1; i += 1) {
    longestWord = words[i];
    if (words[i + 1].length > longestWord.length) {
...
function extractNameFromTemplate(value) {
  const re = /[!,]/g;
  return value.replace(re, '').split(' ').slice(1).join(' ');
}
